...
ticketPriceTotal[0] = runners.reduce((donation, item) => {
    return donation += item.donation}, 0)
...
// Problem 1 - Create an email list of all the runners to make it easier to email everyone
let emailList = [];
emailList = runners.map(item => {
    return item.email;
})

console.log(emailList);

// Problem 2 - Count the Shirt Sizes so we know how many to order in each size
let shirtSizeCount = {};
runners.forEach((item) => {
    if (!shirtSizeCount.hasOwnProperty(item.shirt_size)) {
        shirtSizeCount[item.shirt_size] = 1;
    }
    else shirtSizeCount[item.shirt_size] += 1;
} )
